...
**Development project** — the `dev` profile workspace (`profiles/dev/Cargo.toml`), generated by
`cargo polylith profile migrate` (or `cargo polylith cargo`). It includes all components, bases,
and projects as members and is optimised for fast feedback during development and testing.
Components default to lightweight implementations (stubs, in-memory versions); profiles select
...
# → writes profiles/dev.profile and generates profiles/dev/Cargo.toml
# → clears root [workspace].members (bricks are now members of the profile workspace)

# 9. Build via the profile workspace
...
```
my-mono/
  Cargo.toml              ← root manifest; [workspace].members is empty after profile migrate
  ...
  projects/
    production/
      Cargo.toml          ← bin crate (profile workspace member); [dependencies] selects real components
      src/main.rs
    bdd/
      Cargo.toml          ← test/dev project (profile workspace member); [dependencies] uses stubs
      src/main.rs
  profiles/
