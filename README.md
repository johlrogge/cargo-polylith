...
Generates symlinks and `profiles/<name>/Cargo.toml` — a standalone profile workspace
that applies the named profile's implementation overrides to the root `[workspace.dependencies]`.
Member paths in the generated `Cargo.toml` use the symlinked layout (`components/foo`)
rather than relative paths (`../../components/foo`), satisfying Cargo 1.94+'s requirement
that members be hierarchically below the workspace root. Optionally invokes `cargo build`
inside that generated workspace.
...
3. Creates symlinks inside `profiles/dev/`: `components → ../../components`,
   `bases → ../../bases`, `projects → ../../projects`
4. Generates `profiles/dev/Cargo.toml` using clean member paths (`components/foo`)
   that are valid under Cargo 1.94+'s requirement that members be hierarchically below
   the workspace root
5. Strips `{ workspace = true }` from brick `Cargo.toml`s so they are self-contained
6. Clears root `[workspace] members` to `[]`
...
place of direct `cargo` invocations. You can also `cd` into the profile directory and
run cargo directly:

```
cargo polylith cargo check          # generates profile workspace and delegates to cargo
cargo polylith cargo build
cargo polylith cargo test

cd profiles/dev && cargo check      # equivalent — work directly in the profile workspace
```
...
You can also work directly inside the profile directory after running `profile migrate`
(or after any `cargo polylith cargo` invocation, which regenerates it):

```
cd profiles/dev && cargo check
```
...
my-mono/
  ...
  profiles/
    ...
    dev/
      ...
      Cargo.toml          ← generated; member paths: "components/foo" (valid under Cargo 1.94+)
    production/
      components -> ../../components   (symlink)
      bases      -> ../../bases        (symlink)
      projects   -> ../../projects     (symlink)
      Cargo.toml          ← generated by `cargo polylith cargo --profile production build`
```
