...
- **`cargo polylith bump [major|minor|patch]`** — bumps the workspace version in `Polylith.toml` and the root `Cargo.toml` `[workspace.package]` version. Relaxed mode only; strict mode is planned.
