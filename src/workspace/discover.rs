...
pub fn discover_profiles(root: &Path) -> Result<Vec<Profile>> {
    ...
    for entry in std::fs::read_dir(&profiles_dir).map_err(io_err(&profiles_dir))? {
        ...
        // Parse [profile.*]: clone each child table for pass-through into root Cargo.toml
        let mut profiles_map = std::collections::BTreeMap::new();
        if let Some(profile_tbl) = doc.get("profile").and_then(|p| p.as_table()) {
            for (k, v) in profile_tbl.iter() {
                if let Some(tbl) = v.as_table() {
                    profiles_map.insert(k.to_string(), tbl.clone());
                }
            }
        }
        profiles.push(Profile { name, path: path.clone(), implementations, libraries, profiles: profiles_map });
    }
...
pub fn resolve_profile_workspace(
    ...
    // Render [profile.*] sections from the .profile file into verbatim TOML blocks
    let cargo_profile_sections: Vec<String> = profile.profiles.iter().map(|(name, table)| {
        let mut fresh = toml_edit::DocumentMut::new();
        fresh["profile"] = toml_edit::Item::Table({
            let mut outer = toml_edit::Table::new();
            outer.insert(name, toml_edit::Item::Table(table.clone()));
            outer
        });
        fresh.to_string().trim_end().to_string()
    }).collect();
    ...
    ResolvedProfileWorkspace {
        ...
        library_dep_lines,
        workspace_package,
        cargo_profile_sections,
    }
}

/// Read the root `Cargo.toml` and return the names of any top-level `[profile.*]` sections.
/// Returns an empty vec when the file does not exist or has no `[profile]` table.
///
/// If the file begins with our generated-file header (`# GENERATED BY cargo-polylith`),
/// any `[profile.*]` sections in it were emitted by a previous `change-profile` run and
/// are therefore not orphaned — we return an empty vec immediately in that case.
pub fn detect_orphaned_cargo_profiles(root: &Path) -> Result<Vec<String>> {
    let cargo_toml_path = root.join("Cargo.toml");
    if !cargo_toml_path.exists() {
        return Ok(vec![]);
    }
    let content = fs::read_to_string(&cargo_toml_path).map_err(io_err(&cargo_toml_path))?;
    // If we wrote this file ourselves, every [profile.*] section came from a .profile
    // by definition — nothing is orphaned.
    if content.starts_with("# GENERATED BY cargo-polylith") {
        return Ok(vec![]);
    }
    let doc: toml_edit::DocumentMut = content.parse().map_err(parse_err(&cargo_toml_path))?;
    let names = doc
        .get("profile")
...
mod tests {
    ...
    #[test]
    fn discover_profiles_parses_profile_sections() {
        use tempfile::TempDir;
        use std::fs;
        ...
        fs::write(
            profiles_dir.join("prod.profile"),
            r#"[implementations]
store = "components/store_file"
...
"#,
        ...
        let profiles = discover_profiles(dir.path()).unwrap();
        let prod = profiles.iter().find(|p| p.name == "prod").expect("expected prod profile");

        assert!(prod.profiles.contains_key("release"), "should have profile.release");
        assert!(prod.profiles.contains_key("dev"), "should have profile.dev");

        let release = &prod.profiles["release"];
        assert!(
            release.get("strip").and_then(|v| v.as_value()).and_then(|v| v.as_bool()) == Some(true),
            "profile.release.strip should be true"
        );
    }
